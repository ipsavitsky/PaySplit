...
import { getSplitPayPlugin } from "../src/utils/contracts";
...
describe("RelayPlugin", async () => {
    ...
    const setup = deployments.createFixture(async ({ deployments }) => {
        ...
        const plugin = await getSplitPayPlugin(hre);
        return {
            account,
            plugin,
            manager,
    ...
    it("should be a success if amount is correct", async () => {
        ...
        let tx = (await plugin.executeFromPlugin.populateTransaction(await manager.getAddress(), account, await account.getAddress(), 1000));
        await expect(relayer.sendTransaction({ to: plugin, data: tx.data, value: 500})).to.not.be.reverted;
